...
export default abstract class AbstractAlertMonitor implements IMonitor {
    ...
    public startMonitoring(): void {
        if (!this.checkFrequency) {
            ...
            throw new Error('checkFrequency cannot be less than or equal to zero');
            ...
            throw new Error('refreshMetricsFrequency cannot be less than or equal to zero');
        }
        setInterval(async () => {
        ...
        setInterval(() => { this.refreshMetrics(); }, this.refreshMetricsFrequency);
    }
}